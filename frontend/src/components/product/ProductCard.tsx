...
const StarIcon = ({ filled }: { filled: boolean }) => (
  <svg className={`w-3.5 h-3.5 ${filled ? "text-warning" : "text-gray-300"}`}
...
export default function ProductCard({ product: p }: Props) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem(p, 1);   // CartContext nhận ProductSummaryDto trực tiếp
    setAdded(true);
    ...
    >
      ...
      <div className="relative overflow-hidden bg-[#f7f5f2]" style={{ height: 320 }}>
        {p.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg width="90" height="110" viewBox="0 0 24 24"
                 fill="none" stroke="rgba(0,0,0,0.1)" strokeWidth="0.8">
        ...
        <button onClick={handleWishlist}
          ...
          <svg className={`w-4 h-4 transition-all ${wished ? "fill-red-500 stroke-red-500" : "fill-none stroke-gray-500"}`}
               viewBox="0 0 24 24">
      ...
      <div className="px-5 pt-4 pb-5">
        ...
        <div className="flex items-center gap-1 mb-3">
          {Array.from({ length: 5 }).map((_, i) => <StarIcon key={i} filled={i < 4} />)}
