...
const SignUp = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    // ── Validate phía client (chỉ check rỗng) ──
    ...
    try {
      ...
      setSuccessMsg(
        `Hello dear ${firstName}, Welcome you to HAUSHOP! Your account has been created successfully.`
      );
      setFirstName("");
      setLastName("");
      setEmail("");
      ...
      if (e.errors && Object.keys(e.errors).length > 0) {
        ...
        // Nếu có errors nhưng không match field nào → hiện server error thay vì im lặng
      ...
      <div className="w-full lgl:w-[500px] h-full flex flex-col justify-center">
        {successMsg ? (
          <div className="w-[500px]">
            <div className="w-full px-4 py-6 flex items-start gap-3 bg-green-50 border border-green-200 rounded-md mx-4">
              <svg className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
              </svg>
              <p className="text-green-700 font-medium font-titleFont text-sm">
                {successMsg}
              </p>
            </div>
            <Link to="/signin">
              <button className="w-full h-10 bg-primeColor rounded-md text-gray-200 text-base font-titleFont font-semibold tracking-wide hover:bg-black hover:text-white duration-300 mt-4 mx-4" style={{ width: "calc(100% - 2rem)" }}>
                Sign in
              </button>
            </Link>
          </div>
          ...
          >
            <div className="px-6 py-4 w-full flex flex-col justify-start overflow-y-auto">
              ...
              <div className="flex flex-col gap-3">
                ...
                {serverErrMsg && (
                  <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-md px-4 py-3">
                    ...
                    <p className="text-sm text-red-600 font-titleFont font-semibold">
                      {serverErrMsg}
                    </p>
                  </div>
                )}
